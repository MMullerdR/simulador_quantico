...
[dgm.cpp:443](../src/core/dgm.cpp#L443):
...
A parte não óbvia é `pos0 = (pos*2) - (pos & (shift-1))`. Isso é um truque
para "inserir um bit 0 na posição `end` de `pos`", **sem** fazer dois shifts
variáveis (`((pos >> end) << (end+1)) | (pos & (shift-1))`, que seria a forma
óbvia mas mais cara). Prova rápida com números pequenos: se `end=2`
(`shift=4`), separe `pos` em "parte alta" `H` e "2 bits baixos" `L`
(`pos = 4H + L`). Então `pos*2 - L = 8H + 2L - L = 8H + L`, que é exatamente
`H` deslocado 3 bits (abriu espaço no bit 2) mais `L` nos 2 bits baixos — ou
seja, o mesmo valor que inserir um `0` no bit 2 de `pos`. `pos1` é só
`pos0` com o bit do qubit alvo ligado (`| shift`).
...
os bits de controle (`ctrl_mask`) e o bit alvo (`shift`) são **fixos**
(controle = `ctrl_value`, alvo começa em 0); só os bits "livres" (nem
controle, nem alvo) variam. `mask` marca esses bits livres. A expressão
...
percorrer todo o vetor e testar `if ((pos & ctrl_mask) == ctrl_value)` a cada
posição — que é exatamente o que a versão mais simples (e mais lenta)
`CpuExecution2_1` faz (ver [07](07-bugs-e-pontos-de-atencao.md), essa versão
está desconectada da execução, mantida só como referência/histórico).

## 4. Os três "tipos de matriz" — a otimização mais importante

`PT::matrixType()` ([common.cpp:24](../src/core/common.cpp#L24)) olha
...
(`CpuExecution1_2`, [dgm.cpp:476](../src/core/dgm.cpp#L476)):

```c
state[pos] = pt->matrix[((pos >> shift) & 1) * 3] * state[pos];
```

(`(pos>>shift)&1` é o bit do qubit alvo; `*3` escolhe `matrix[0]` ou
`matrix[3]`, os dois elementos da diagonal). Isso custa **1 multiplicação por
amplitude**, contra as 4 multiplicações + 2 somas do caso denso.
...
(`CpuExecution1_3`, [dgm.cpp:494](../src/core/dgm.cpp#L494)) — metade
das multiplicações do caso denso.

`DGM::CpuExecution1(it)` ([dgm.cpp:417](../src/core/dgm.cpp#L417)) só
...
[dgm.cpp:789](../src/core/dgm.cpp#L789). Ideia: em vez de rodar um `PT`
...
processada **de forma totalmente independente** por uma thread diferente,
desde que todos os `PT`s daquele lote de operações só toquem qubits dentro
da região (por isso o código primeiro agrupa quantos `PT`s consecutivos
"cabem" em uma região de tamanho `cpu_region`, olhando os qubits que cada um
afeta, antes de disparar as threads OpenMP).

Dentro da região, `PCpuExecution1_0` ([dgm.cpp:877](../src/core/dgm.cpp#L877))
...
`cpu_region` e `cpu_coales` são os parâmetros de tuning:
- `cpu_region`: quantos qubits (bits) cabem "dentro" de uma região processada
  de uma vez por uma thread antes de trocar de região.
- `cpu_coales`: quantos desses bits são tratados como o "núcleo coalescido"
  (os bits menos significativos, sempre presentes na região, ajudando a
  manter acesso à memória mais sequencial/cache-friendly).

## 6. Nota sobre `CpuExecution2_*` e `CpuExecution3_*`

O arquivo tem três "famílias" de funções (`CpuExecution1_*`, `_2_*`, `_3_*`)
implementando o mesmo cálculo de formas diferentes — aparentam ser
experimentos de otimização feitos durante a pesquisa. **Apenas a família `1`
está conectada** (`DGM::execute()` só chama `CpuExecution1`). As famílias `2`
e `3` (essa última com um sistema mais elaborado de "gap"/"max" para pular
sobre blocos contíguos de bits livres) existem no binário mas nunca são
chamadas — ver [07-bugs-e-pontos-de-atencao.md](07-bugs-e-pontos-de-atencao.md).
